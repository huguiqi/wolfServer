...
public final class PKSigningUtil {
    ...
    private static final String CERT_P12_NAME = "Cert.p12";
    private static final String WWDR_PEM_NAME = "WWDR.pem";
    private static final String CERT_RAW = "certAbout";


    private static final Logger LOGGER = LoggerFactory.getLogger(PKSigningUtil.class);
    ...
    public static void signManifestFile(final File temporaryPassDirectory, final File manifestJSONFile,
        ...
        if (temporaryPassDirectory == null || manifestJSONFile == null || signingInformation == null || !signingInformation.isValid()) {
            LOGGER.error("-----signManifestFile()----Null params are not supported----");
    ...
    public static PKSigningInformation loadSigningInformationFromPKCS12FileAndIntermediateCertificateFile(final String passbookDirPath,
        ...
        String p12Path = passbookDirPath +File.separator + CERT_RAW + File.separator + CERT_P12_NAME;
        String appleWWDR_Path = passbookDirPath +File.separator + CERT_RAW + File.separator  + WWDR_PEM_NAME;

        KeyStore pkcs12KeyStore = loadPKCS12File(p12Path, keyStorePassword);
        ...
        X509Certificate appleWWDRCACert = loadDERCertificate(appleWWDR_Path);
        if (signingCert == null || signingPrivateKey == null || appleWWDRCACert == null) {
            LOGGER.error("-----loadSigningInformationFromPKCS12FileAndIntermediateCertificateFile()----Couldnot load all the neccessary certificates/keys----");
            throw new IOException("Couldnot load all the neccessary certificates/keys");
        }
    ...
    public static X509Certificate loadDERCertificate(final String filePath) throws IOException, CertificateException {
        InputStream certificateFileInputStream = null;
        try {
            ...
            LOGGER.error("-----The key from '" + filePath.toString() + "' could not be decrypted----");
            throw new IOException("The key from '" + filePath.toString() + "' could not be decrypted");
        } catch (IOException ex) {
            LOGGER.error("-----The key from '" + filePath.toString() + "' could not be decrypted----",ex);
            throw new IOException("The key from '" + filePath.toString() + "' could not be decrypted", ex);
