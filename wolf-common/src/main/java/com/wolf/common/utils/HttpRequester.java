...
public class HttpRequester {
    ...
    public HttpResponser sendGet(String urlString, Map<String, String> params) throws IOException {
        String fullUrl = this.getFullUrl(urlString, params);
        String content = restTemplate.getForObject(fullUrl, null, String.class);
     ...
     * @throws java.io.IOException
     */
    public HttpResponser sendPost(String urlString, Map<String, String> parameters) {
        String fullUrl = this.getFullUrl(urlString, parameters);
        String content = restTemplate.postForObject(fullUrl, null, String.class);
        HttpResponser httpResponser = new HttpResponser();
        httpResponser.setContent(content);
        return httpResponser;
    }

    public String getFullUrl(String url, Map<String, String> urlVariables) {
        String full_url = null;
        if (urlVariables != null) {
            StringBuffer params = new StringBuffer();
            params.append("?");
            for (Iterator<String> iter = urlVariables.keySet().iterator(); iter
                    .hasNext();) {
                ...
                if (!"null".equals(value) && StringUtils.isNotBlank(value)) {
                    params.append(key);
                    params.append("=");
                    params.append(urlVariables.get(key));
                    if (iter.hasNext())
                        params.append("&");
                }
            }
